...
#region log_relative
        ...
        public class backWork
        {
            public class CommandData
            {
                ...
                public string read_main_bd = "fd ble_binprop get own_identity_address";
                public string read_dumi_bd = "fd ble_binprop get own_identity_address";
                public string write_main_bd = "fd ble_binprop set own_identity_address ";
                public string write_dumi_bd = "fd ble_binprop set lr_set_identity_address ";
                public string write_main_bd_type = "fd ble_binprop set own_identity_address_type 1";
                public string write_dumi_bd_type = "fd ble_binprop set lr_set_identity_address_type 1";
                public string ecomode_disable = "fd app ecomode disable";
                ...
                public string factory_flag_4 = "fd app tracelog write 4 1";
                public string factory_flag_5 = "fd app tracelog write 5 1";
                public string factory_flag_6 = "fd app tracelog write 6 1";
                public string factory_flag_7 = "fd app tracelog write 7 1";
                public string factory_flag_8 = "fd app tracelog write 8 1";
                public string factory_flag_9 = "fd app tracelog write 9 1";
                public string factory_flag_10 = "fd prop set unique ***** 10 1";
                public string factory_flag_11 = "fd prop set unique ***** 11 1";
                public string set_prop = "setprop -f";
            ...
            private bool procTestSequence(string seqName, int index)
            {
                ...
                else if (seqName == "ECOMODE_DISABLE") { retVal = procTestEcomodeDisable(index); }           
                else if (seqName == "FACTORY_FLAG_CLEAR") { retVal = procTestFactoryFlagClear(index); }
                else if (seqName == "FACTORY_FLAG_WRITE") { retVal = procTestFactoryFlagWrite(index); }
                else if (seqName == "SETTING_SAVE") { retVal = procTestSettingSave(index); }

                else if (seqName == "READ_BD_ADDRESS") { retVal = procTestReadMainBAddress(index); }
                else if (seqName == "APPMODE_OUT") { retVal = procTestAppmodeOut(index); }
                else if (seqName == "CARRIER_TRANSMISSION_ON") { retVal = procTestCarrierTransmissionOn(index); }
                else if (seqName == "CARRIER_TRANSMISSION_OFF") { retVal = procTestCarrierTransmissionOff(index); }
                else if (seqName == "CAPID_WRITE_UPDOWN") { retVal = procTestCapidWriteUpdown(index); }
                else if (seqName == "ANALOG_NV_WRITE") { retVal = procTestNvWrite(index); }
            ...
            {                
                ...
                try
                {
                    mainForm.SendMessage(command_data.write_main_bd + "0x"+generatebdstr);
                }
                finally
                {
            ...
            public bool procTestWriteDumiBDAddress(int index)
            {
                ...
                try
                {
                    mainForm.SendMessage(command_data.write_dumi_bd + "0x" + generatedumibdstr);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
            ...
            public bool procTestMainBDAddressType(int index)
            {
                ...
                try
                {
                    mainForm.SendMessage(command_data.write_main_bd_type);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
            ...
            public bool procTestDumiBDAddressType(int index)
            {
                ...
                try
                {
                    mainForm.SendMessage(command_data.write_dumi_bd_type);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
            ...
            public bool procTestEcomodeDisable(int index)
            {
                int flag_ng = 0;

                try
                {
                    mainForm.SendMessage(command_data.ecomode_disable);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Ecomode Disable Fail";
                    return false;
                }
                else
                    return true;
            }
            public bool procTestFactoryFlagClear(int index)
            {
                int flag_ng = 0;
                try
                {
                    mainForm.SendMessage(command_data.factory_flag_clear);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Factory Flag Clear Fail";
                    return false;
                }
                else
                    return true;
            ...
            public bool procTestSettingSave(int index)
            {
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Setting Save Reboot Fail";
                    return false;
                }
                else
                    return true;
            }
            public bool procTestReadMainBAddress(int index)
            {
                int flag_ng = 0;
                try
                {
                    mainForm.SendMessage(command_data.read_main_bd);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Main BD Address Read Fail";
                    return false;
                }
                else
                    return true;
            ...
            public bool procTestAppmodeOut(int index)
            {
                int flag_ng = 0;
                try
                {
                    mainForm.SendMessage(command_data.app_mode_out);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Appmode Out Fail";
                    return false;
                }
                else
                    return true;
            }
            public bool procTestCarrierTransmissionOn(int index)
            {
                int flag_ng = 0;

                try
                {
                    mainForm.SendMessage(command_data.carrier_transmission_on);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Carrier Transmission On Fail";
                    return false;
                }
                else
                    return true;
            }
            public bool procTestCarrierTransmissionOff(int index)
            {
                int flag_ng = 0;

                try
                {
                    mainForm.SendMessage(command_data.carrier_transmission_off);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Carrier Transmission Off Fail";
                    return false;
                }
                else
                    return true;
            ...
            public bool procTestNvWrite(int index)
            {
                int flag_ng = 0;
                
                try
                {
                    mainForm.SendMessage(command_data.rfanalog_nvwrite);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Analog NV Write Fail";
                    return false;
                }
                else
                    return true;
            }
            public bool procTestAppmodeInit(int index)
            {
                int flag_ng = 0;

                try
                {
                    mainForm.SendMessage(command_data.appmode_init);
                }
                finally
                {
                    if (mainForm.Check_Data(index) == false)
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Appmode Init Fail";
                    return false;
                }
                else
                    return true;
            }
            public bool procTestSetProp(int index)
            {
                int flag_ng = 0;

                try
                {
                    mainForm.SendMessage(command_data.set_prop);
                }
                finally
                {
                ...
                if (flag_ng == 1)
                {
                    mainForm.gNgType[index] = "Set Prop Fail";
                    return false;
                }
                else
                    return true;
